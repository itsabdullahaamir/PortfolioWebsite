...
| `portrait.jpg` | 🚧 | Headshot for the `/plain` Dossier identity block (`src/screens/PlainResume.jsx`, driven by `profile.photo` in `src/data/profile.js`). Served at the site root, resolved in the component against `import.meta.env.BASE_URL` so it works under the `/PortfolioWebsite/` subpath. **Currently a generated placeholder** — 400×400, "AA" in `--bone` on a `--shadow` field — so the frame isn't empty; the site owner replaces it with a real photo at this exact path (any square-ish JPG/PNG/WebP; the component renders it `grayscale` to hold the duotone palette). If the file is ever absent the component hides the frame rather than showing a broken image. |
